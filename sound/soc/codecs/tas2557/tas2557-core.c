...
static unsigned int p_tas2557_startup_data[] = {
	...
	TAS2557_RAMP, 0x7FFFFFFF,	/* Volume ramp back up */
	TAS2557_UDELAY, 2000,		/* delay */
	TAS2557_SKEW_RAMP, 0x00221D00,	/* SKEW RAMP UP */
...
#define FIH_E2P_PARTITION_DEVI     "/dev/block/bootdevice/by-name/deviceinfo"
#define E2P_OFFSET_DEVI            36*1024    // 36K
#define SNS_REG_MAGIC_V01     0x55AA
#define SNS_REG_MAGIC_V02     0x5A5A
#define SNS_REG_MAGIC_TAIL    0xA5A5

static char fih_e2p_file[255]= FIH_E2P_PARTITION_DEVI;
	unsigned char pfwBuffer[2048]={0};

struct fih_sns_reg_info{
    uint16_t Magic;
    uint16_t Cal_Info;
    uint32_t len;
};
struct fih_sns_reg_info_tail{
    uint16_t Magic;
    uint16_t Reserved;
};

// ----------------------------------------------------------------------------- 
// Write data into FIH device_info
// ----------------------------------------------------------------------------- 
// Description: 
//      Write calibration data into FIH device_info
// ----------------------------------------------------------------------------- 
static int tas2557_load_calibration(struct tas2557_priv *pTAS2557,	char *pFileName)
{
#if 0 // read calibration data from data/tas2557_cal.bin
	int nResult = 0;
  struct file *fp = NULL;
	//int nFile;
	mm_segment_t fs;
	...
	fp = filp_open(pFileName, O_RDWR |O_NONBLOCK, 0);
	
  if (IS_ERR(fp)) {
		dev_err(pTAS2557->dev, "Open file %s Error\r\n", pFileName);
		set_fs(fs);
    goto end;
	}
  ...
  pos = fp->f_pos;
  nSize = vfs_read(fp, (unsigned char __user *)&pBuffer, sizeof(pBuffer), &pos);
  ...
  filp_close(fp, NULL);
	...
	if (!nSize)
	{
		dev_err(pTAS2557->dev, "Calibrarion data = 0\r\n");
		goto end;
	}
...
#else //read calibration data from e2p

	int nResult = 0;
	int nSize = 0;
  struct fih_sns_reg_info reginfo;
  struct fih_sns_reg_info_tail reginfo_tail;
  //int fd = -1;
  struct file *fp = NULL;
  ...
  pos = fp->f_pos;
  len = vfs_read(fp, (unsigned char __user *)&reginfo, sizeof(reginfo), &pos);
  
  //printf("Magic=%x,Cal_Info=%x,Len=%d\r\n",reginfo.Magic,reginfo.Cal_Info,reginfo.len);
  dev_info(pTAS2557->dev, "Magic=0x%x,Cal_Info=0x%x,Len=%d\r\n",reginfo.Magic,reginfo.Cal_Info,reginfo.len);
    
  fp->f_op->llseek(fp, E2P_OFFSET_DEVI+64, SEEK_SET);
  pos = fp->f_pos;
  
  if (reginfo.len > 0)
  {
      nSize = vfs_read(fp, (unsigned char __user *)&pfwBuffer, reginfo.len, &pos);
      if (nSize != reginfo.len)
      {
      	  dev_err(pTAS2557->dev, "Open E2P file length:%d, only read data length:%d\r\n", reginfo.len, nSize);
      	  filp_close(fp, NULL);
	        set_fs(oldfs);
          goto end;	  
      }
...
#if 0 //dump calibration data     
      {
      	  ...
      	  for (i=0; i<((nSize/16)+1); i++)
      	  {
      	  	   dev_info(pTAS2557->dev, "0x%08x: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\r\n", i*16, 
      	  	           pfwBuffer[i*16], pfwBuffer[i*16+1], pfwBuffer[i*16+2], pfwBuffer[i*16+3], pfwBuffer[i*16+4], pfwBuffer[i*16+5], pfwBuffer[i*16+6], pfwBuffer[i*16+7],
      	  	           pfwBuffer[i*16+8], pfwBuffer[i*16+9], pfwBuffer[i*16+10], pfwBuffer[i*16+11], pfwBuffer[i*16+12], pfwBuffer[i*16+13], pfwBuffer[i*16+14], pfwBuffer[i*16+15]);
      	  }
      }
#endif
      
      dev_info(pTAS2557->dev, "E2P file read successfully (nSize:%d)\r\n", nSize);
  }
  
  fp->f_op->llseek(fp, E2P_OFFSET_DEVI+64+reginfo.len, SEEK_SET);
  pos = fp->f_pos;
  len = vfs_read(fp, (unsigned char __user *)&reginfo_tail, sizeof(reginfo_tail), &pos);
  //printf("Tail info1=%x\r\n",reginfo_tail.Magic);
  dev_info(pTAS2557->dev, "Tail info1=0x%x\r\n",reginfo_tail.Magic);
  
  filp_close(fp, NULL);
	set_fs(oldfs);

  if (reginfo.Magic!=SNS_REG_MAGIC_V01)
  {
  	  dev_err(pTAS2557->dev, "Open E2P file fail: wrong tail Magic:0x%x \r\n", reginfo.Magic);
      goto end;	  
  }
  
  if (reginfo_tail.Magic!=SNS_REG_MAGIC_TAIL)
  {
  	  dev_err(pTAS2557->dev, "Open E2P file fail: wrong tail Magic:0x%x \r\n", reginfo_tail.Magic);
      goto end;	  
  }
...
void tas2557_fw_ready(const struct firmware *pFW, void *pContext)
{
  ...
  //20170901@ray: loading calibration data
  nResult = tas2557_set_calibration(pTAS2557, 0xFF);
  if (nResult < 0) {
		dev_err(pTAS2557->dev, "There is no calibration data!\n");
	}
...
int tas2557_set_calibration(struct tas2557_priv *pTAS2557, int nCalibration)
{
	...
	dev_dbg(pTAS2557->dev, "tas2557_set_calibration() - enter\n");
...
int tas2557_parse_dt(struct device *dev, struct tas2557_priv *pTAS2557)
{
	...
	rc = of_property_read_string(np, "tas2557,device-model", &device_model);

  if (!rc) {
     if (strcmp(device_model, "B2N") == 0) {
     	    //EVT samples
          tas2557_fw_bin = "TAS2557MSSMono_B2N.bin";

      }
      else if (strcmp(device_model, "B2N_dvt") == 0) {
      	  //DVT, PVT ....,etc samples
          if(strstr(saved_command_line, "mdss_mdp.panel=1:dsi:0:fih,mdss_dsi_nt36672_fhd_video")) {
          	//Second source speaker
                // 20180307-second source speaker >>>
                pTAS2557->speaker_type = 2;
	        // 20180307-second source speaker <<<
          	tas2557_fw_bin = "TAS2557MSSMono_B2N_dvt_ICTspk.bin";
          	dev_err(pTAS2557->dev, "tas2557 with ICT speaker\n");
          }
          else if (strstr(saved_command_line, "mdss_mdp.panel=1:dsi:0:fih,mdss_dsi_ft8716u_fhd_video")) {
                 //Main source speaker
                 // 20180307-second source speaker >>>
                 pTAS2557->speaker_type = 1;
	        // 20180307-second source speaker <<<
          	 tas2557_fw_bin = "TAS2557MSSMono_B2N_dvt.bin";
          	 dev_err(pTAS2557->dev, "tas2557 with main source speaker\n");
          }
          else {
          	 //Unmount speaker, board level
          	 tas2557_fw_bin = "TAS2557MSSMono_B2N_dvt.bin";
          	 dev_err(pTAS2557->dev, "tas2557 without speaker\n");
          }		
      }
     else if (strcmp(device_model, "DRG") == 0) {
     	    //EVT samples
          tas2557_fw_bin = "TAS2557MSSMono_DRG.bin";

      }
     else if (strcmp(device_model, "CTL") == 0) {
     	    //EVT samples
          tas2557_fw_bin = "TAS2557MSSMono_CTL.bin";

      }
      /* Other project need to add related firmware name */
      //else if (strcmp(device_model, "PL2") == 0) {
      //    tas2557_fw_bin = "TAS2557MSSMono_PL2.bin";
      //}
      else {
      	  tas2557_fw_bin = "TAS2557MSSMono.bin";
      }
  }
  dev_err(pTAS2557->dev, "tas2557_fw_bin=%s\n", tas2557_fw_bin);
